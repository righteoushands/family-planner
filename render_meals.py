...
def render_meal_planner_page(status: str = "", week_key: str = None) -> str:
    ...
    body = (
        ...
        f'var _apiKey = "";'
        f'var _wakeLock = null;'
        f'var _micRecog = null;'
        f'var _micActive = false;'

        # Wake lock: request when page is visible, release when hidden
        'async function _requestWakeLock() {'
        '  try { if (navigator.wakeLock) { _wakeLock = await navigator.wakeLock.request("screen"); } } catch(e) {} }'
        ...
        '  if (_micActive) { if (_micRecog) _micRecog.stop(); return; }'
        '  var SpeechRec = window.SpeechRecognition || window.webkitSpeechRecognition;'
        '  if (!SpeechRec) { alert("Speech recognition not supported in this browser. Try Chrome or Safari."); return; }'
        '  _micRecog = new SpeechRec();'
        '  _micRecog.continuous = true;'
        '  _micRecog.interimResults = true;'
        '  _micRecog.lang = "en-US";'
        '  var _committed = ta.value;'
        ...
        '      if (e.results[i].isFinal) { _committed += e.results[i][0].transcript; }'
        '      else { interim = e.results[i][0].transcript; }'
        '    }'
        '    ta.value = _committed + interim;'
        '  };'
        '  _micRecog.onend = function() {'
        '    _micActive = false;'
        '    btn.textContent = "\U0001F3A4 Dictate";'
        ...
        '  if (chunks.use_soon.length) { document.getElementById("inv-use-soon").value = chunks.use_soon.join(", "); filled++; }'
        '  var st = document.getElementById("inv-parse-status");'
        '  if (st) {'
