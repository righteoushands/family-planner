...
def render_meal_print_page(week_key: str = None) -> str:
    ...
    return (
        ...
        "padding:8px 16px;border-radius:6px;cursor:pointer;font-size:13px;font-weight:700;'>&#128438; Print</button>"
...
def render_recipes_page(status: str = "") -> str:
    ...
    add_form = (
        ...
        '<form method="POST" action="/recipe-import">'
        '<div style="margin-bottom:14px;">'
        '<label>Recipe URL</label>'
        '<input type="url" name="url" placeholder="https://www.seriouseats.com/..." style="margin-bottom:8px;">'
        '<p style="font-size:0.78em;color:var(--ink-faint);">Paste a link and we\'ll try to extract the recipe.</p>'
        '</div>'
        '<div style="margin-bottom:14px;">'
        '<label>Or paste recipe text</label>'
        '<textarea name="text" rows="6" placeholder="Paste any recipe text, ingredients list, or instructions..." '
        ...
        'border-radius:8px;font-family:inherit;font-size:0.85em;cursor:pointer;color:var(--ink-muted);">Cancel</button>'
        '</div></form>'
        '<div style="margin-top:10px;padding:10px 14px;background:#faf8f5;border-radius:8px;">'
        '<p style="font-size:0.78em;color:var(--ink-faint);margin:0;">'
        '\U0001f4f8 <strong>Photo or document?</strong> '
        'Take a photo of a recipe card or cookbook page, copy the text, and paste it above. '
        'The app will parse it into ingredients and instructions.'
        '</p></div>'
        '</div>'
        '</div>'
    ...
    body = (
        ...
        '  var el = document.getElementById("recipe-detail-"+id);'
